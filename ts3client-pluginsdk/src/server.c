...
int send_voice(short* samples, int sample_counter, int channels){
    ...
    printf("done, sending...\n");
    printf("total size to send: %i expected send: %i\n", odone*2, (odone*2)/UDP_SIZE);
    
    int sent = 0;
    int mycounter = 1;
    while (sent < odone*2){
      int available = odone*2 - sent > UDP_SIZE ? UDP_SIZE : odone*2 - sent;
      ...
      if (sendto(socket_desc, data, odone*2, 0,
                (const struct sockaddr_in*)&client_addr, sizeof(client_addr)) < 0)
      {
